...
public class Torrent {
	...
	// class vars
	/*
	 * private String id; private String fileName; private String title; private
	 * boolean freeLeech = false; private boolean newTorrent = false; private
	 * String descriptionUrl; private String categoryUrl; private String
	 * category; private String downloadUrl; private String dateAdded; private
	 * String size; private String seeders; private String leechers; private
	...
	// getters
	public String getId() {
		return mInfo.getString("id");
	}

	public String getFileName() {
		return mInfo.getString("fileName");
	}

	public String getTitle() {
		return mInfo.getString("title");
	}

	public boolean isFreeLeech() {
		return mInfo.getBoolean("freeLeech");
	}

	public boolean isNew() {
		return mInfo.getBoolean("new");
	}

	public String getDescriptionUrl() {
		return baseUrl + mInfo.getString("descriptionUrl");
	}

	public String getCategory() {
		return mInfo.getString("category");
	}

	public String getCategoryName() {
		return LM.getInstance().getCategories()
				.get(Integer.parseInt(mInfo.getString("category"))).getName();
	}

	public String getCategoryUrl() {
		return baseUrl + mInfo.getString("categoryUrl");
	}

	public String getDownloadUrl() {
		return baseUrl + mInfo.getString("downloadUrl");
	}

	public String getDateAdded() {
		return mInfo.getString("dateAdded");
	}

	public String getSize() {
		return mInfo.getString("size");
	}

	public String getSeeders() {
		return mInfo.getString("seeders");
	}

	public String getLeechers() {
		return mInfo.getString("leechers");
	}

	public String getFullDescription() {
		return mInfo.getString("fullDescription");
	}

	public String getComments() {
		return mInfo.getString(Const.COMMENT_STRING);
	}

	public String getUploadedBy() {
		return mInfo.getString("uploadedBy");
	}

	public String getFileCount() {
		return mInfo.getString("fileCount");
	}

	public String getThankYous() {
		return mInfo.getString("thankYous");
	}

	public boolean getThanked() {
		return mInfo.getBoolean("thanked");
	}
	...
	public void parseTableRow(Element row) {
		try {
			...
			// downlaod url
			String downloadUrl = titleCell.select("a.index").first()
					.attributes().get("href");
			...
			try {
				...
				Log.e("DEBUG",
						"Failed to parse download URL into id and filename strings. "
								+ e.getMessage());
				e.printStackTrace();
			}
			// free leech
			mInfo.putBoolean("freeLeech", titleCell.select("a:not(.index) img")
					.size() > 0);
			// new
			...
			// category
			String categoryUrl = cells.get(0).select("a").first().attributes()
					.get("href");
			mInfo.putString("categoryUrl", categoryUrl);
			mInfo.putString("category",
			...
			Log.e("DEBUG",
					"Can't parse torrent data to an object. " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	public void parseTorrentInfo(Document doc) {
		try {
			...
			int nfoOffset = rows.get(3 + freeLeechOffset).select("td").get(0)
					.text().contains("NFO") ? 1 : 0;

			mInfo.putString("fullDescription", rows.get(1 + freeLeechOffset)
					.select("td").get(1).html());
	...
	public void open(Activity context) {
		String str = String.format(
				context.getString(R.string.lm_file_downloading), getFileName());
		...
		Runnable openTorrents = new Runnable() {
			...
			public void run() {
				String error = null;
				try {
					final File torrent = LM.getInstance().downloadFile(
							Torrent.this);
					progressDialog.dismiss();

					final String msg = String
							.format(cntxt
									.getString(R.string.lm_file_downloaded_prompt),
									torrent.getAbsolutePath());
											...
											new DialogInterface.OnClickListener() {
												public void onClick(
													...
													try {
														...
														Toast.makeText(
																cntxt,
																R.string.no_application_associated,
																Toast.LENGTH_LONG)
																.show();
														Log.e("DEBUG",
																"Activity associated with this file type not found");
													}
												}
											})
					...
					String str = "While opening torrent MalformedURLException. "
							+ e.getMessage();
					Log.e("DEBUG", str);
					error = str;
					e.printStackTrace();
					...
					String str = cntxt
							.getString(R.string.sd_card_not_avaliable)
							+ " "
							+ e.getMessage();
					Log.e("DEBUG",
							"External storage not avaliable. " + e.getMessage());
					error = str;
					e.printStackTrace();
					...
					error = cntxt.getString(R.string.lm_no_connectivity) + " "
							+ e.getMessage();
					e.printStackTrace();
				} finally {
					progressDialog.dismiss();
				...
				if (error != null) {
					...
					cntxt.runOnUiThread(new Runnable() {
						public void run() {
							Toast.makeText(cntxt, err, Toast.LENGTH_LONG)
									.show();
						}
					});
				}
	...
	public void sendToUTorrent(Activity context) {
		String str = String.format(
				context.getString(R.string.ut_send_in_progress), getFileName());
		final ProgressDialog progressDialog = ProgressDialog.show(context,
				context.getString(R.string.please_wait), str, true);
		final Activity cntxt = context;
		...
		Runnable sendTorrents = new Runnable() {
			...
			public void run() {
				...
				try {
					...
					msg = String.format(
							cntxt.getString(R.string.ut_send_success),
							ut.getLabel());
					...
					msg = cntxt.getString(R.string.ut_cant_connect) + " "
							+ e.getMessage();
					e.printStackTrace();
				} finally {
					progressDialog.dismiss();
	...
	public void sendToTransmission(Activity context) {
		String str = String.format(
				context.getString(R.string.tr_send_in_progress), getFileName());
		final ProgressDialog progressDialog = ProgressDialog.show(context,
				context.getString(R.string.please_wait), str, true);
		final Activity cntxt = context;
		...
		Runnable sendTorrents = new Runnable() {
			...
			public void run() {
				...
				try {
					...
					msg = String.format(
							cntxt.getString(R.string.tr_send_success),
							tr.getLabel());
				} catch (InvalidTokenException e) {
					...
					msg = cntxt.getString(R.string.tr_cant_connect) + " "
							+ e.getMessage();
					e.printStackTrace();
				} finally {
					progressDialog.dismiss();
